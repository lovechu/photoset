...
#### **1. 新增管理模块**
- **分类管理** (`CategoryManage.vue`) — 分类CRUD，支持名称/标识/描述/排序
- **订单管理** (`OrderManage.vue`) — 完整订单列表，支持查看详情、批量操作、退款处理
- **标签管理** (`TagManage.vue`) — 已实现，完善批量操作

#### **2. 统计增强**
- **Dashboard** (`Dashboard.vue`) — 实时统计套图上传量/订单量/销售额/用户增长
